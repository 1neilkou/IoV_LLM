...
def reward_function(completions, **kwargs):
    ...
    for content in completions:
        ...
        if not re.search(r"\[.*\]", content): score -= 2.0
...
def main():
    ...
    training_args = GRPOConfig(
        ...
        num_generations=8,              
        max_completion_length=256,       # 留出足够的显存 buffer
        num_train_epochs=50,
        beta=0.01,
        ...
        logging_dir="./output/iov_qwen_grpo/logs", # 指定日志存放位置
    ...
    # 【修改点 B】: 显式保存训练状态和指标
    # trainer.save_state()  # 这会生成那个你想要的 trainer_state.json
    # trainer.save_metrics("train", trainer.state.log_history) # 保存最终指标汇总
    
    print("✅ 进化完成！模型存至 final_agent，日记存至 trainer_state.json")
